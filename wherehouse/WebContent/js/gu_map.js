...
function initGuSpecE(data) {
    guName.forEach(e => {
        guSpec.push({
            ...
            safety: Math.floor(Math.random() * 10) + 1,
            convenience: Math.floor(Math.random() * 10) + 1,
            convenienceStore: data.cvt_store, // 편의점
            cafe: data.cafe, // 카페
            restaurant: data.restourant,   // 음식점
            olive: data.oliveYoung,    // 올리브영
            daiso: data.daiso,    // 다이소
            polliceOffice: data.police_office,    // 경찰서
            cctv: data.cctv, // cctv
            arrest: data.safe_score    // 검거율
        });
    })
    console.log("guSpec 배열에 들어간 값들 : " + guSpec);
}
